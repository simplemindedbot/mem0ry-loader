...
from src.parsers.json_parser import ChatGPTJSONParser, Conversation, ChatMessage
...
class TestChatGPTJSONParser:
    ...
    def test_get_conversation_chunks_basic(self, sample_conversation_data: Dict[str, Any]):
        """Test basic conversation chunking."""
        parser = ChatGPTJSONParser()
        conversation = parser._parse_conversation(sample_conversation_data)
        
        chunks = list(parser.get_conversation_chunks(conversation, chunk_size=100, overlap=20))
    ...
    def test_get_conversation_chunks_overlap(self):
        ...
        chunks = list(parser.get_conversation_chunks(conversation, chunk_size=100, overlap=50))
    ...
    def test_get_conversation_chunks_different_sizes(self, sample_conversation_data: Dict[str, Any], 
                                                   chunk_size: int, overlap: int):
        """Test chunking with different chunk sizes."""
        parser = ChatGPTJSONParser()
        conversation = parser._parse_conversation(sample_conversation_data)
        
        chunks = list(parser.get_conversation_chunks(conversation, chunk_size=chunk_size, overlap=overlap))
