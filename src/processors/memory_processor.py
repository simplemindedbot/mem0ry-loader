...
class MemoryProcessor:
    ...
    def process_memories(
    ...
    ) -> tuple[List[ExtractedMemory], ProcessingStats]:
        ...
        self.logger.info(
            f"Processing complete. Output: {len(cleaned_memories)} memories"
        )
        return cleaned_memories, stats

    def _filter_by_confidence(
        self, memories: List[ExtractedMemory], stats: ProcessingStats
        ...
        for memory in memories:
            if memory.confidence >= self.confidence_threshold:
                ...
                self.logger.debug(
                    f"Filtered low confidence memory: {memory.content[:50]}..."
                )

        self.logger.info(
            f"Filtered {stats.low_confidence_filtered} low confidence memories"
        )
        return filtered
    ...
    def _merge_similar_memories(
        ...
        for _category, category_memories in category_groups.items():
    ...
    def _are_similar_memories(
        self, memory1: ExtractedMemory, memory2: ExtractedMemory
    ) -> bool:
    ...
    def _merge_memory_group(self, memories: List[ExtractedMemory]) -> ExtractedMemory:
        ...
        # Combine contexts
        combined_context = " | ".join({m.context for m in memories if m.context})
