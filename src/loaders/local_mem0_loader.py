...
class LocalMem0Loader:
    ...
    def __init__(
        self, base_url: str = "http://localhost:8765", user_id: str = "default_user"
    ):
    ...
    def _test_connection(self):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to connect to local OpenMemory server at {self.base_url}: {e}"
            )
            self.logger.info(
                "Make sure OpenMemory server is running with 'make up' in the mem0/openmemory directory"
            )
            raise
    ...
    def load_memories(
        ...
        success_rate = (
            self.uploaded_count / total_processed if total_processed > 0 else 0.0
        )
    ...
    def _upload_single_memory(self, memory: ExtractedMemory):
        ...
        response = requests.post(
            f"{self.base_url}/api/v1/memories/", json=memory_data, timeout=30
        )

        if response.status_code not in [200, 201]:
            self.logger.error(
                f"Failed to upload memory: {response.status_code} - {response.text}"
            )
    ...
    def delete_existing_memories(self, memory_ids: List[str]) -> int:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.warning(
                    f"Failed to delete memories: {response.status_code}"
                )
                return 0
        except requests.exceptions.RequestException as e:
    ...
    def prepare_memories_for_upload(
