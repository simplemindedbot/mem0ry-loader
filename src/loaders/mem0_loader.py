...
class Mem0Loader:
    ...
    def _upload_single_memory(self, memory: ExtractedMemory):
        ...
        try:
            result = self.client.add(
                messages=messages, user_id=self.user_id, metadata=metadata
            )
            ...
            self.logger.error(f"Error uploading memory: {e}")
            raise
    ...
    def get_existing_memories(self) -> List[Dict]:
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(memories)} existing memories for user {self.user_id}"
            )
            return memories
    ...
    def check_for_duplicates(
        ...
        for memory in memories:
            if memory.content not in existing_content:
                ...
                self.logger.debug(
                    f"Skipping duplicate memory: {memory.content[:50]}..."
                )

        self.logger.info(
            f"Filtered {len(memories) - len(unique_memories)} duplicate memories"
        )
    ...
    def prepare_memories_for_upload(
        ...
        self.logger.info(
            f"Removed {len(valid_memories) - len(unique_memories)} internal duplicates"
        )
