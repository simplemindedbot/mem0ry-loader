...
from ..config.settings import settings, OpenAIModel
...
class OpenAIExtractor:
    ...
    def __init__(self, model: OpenAIModel = None, api_key: str = None, use_batch: bool = False):
        self.model = model or settings.openai_model
        self.api_key = api_key or settings.openai_api_key or os.getenv('MEMLOADER_OPENAI_API_KEY')
        self.use_batch = use_batch
        self.logger = logging.getLogger(__name__)
        
        if not self.api_key:
            raise ValueError("OpenAI API key is required. Set MEMLOADER_OPENAI_API_KEY environment variable.")
        ...
        self.logger.info(f"Initialized OpenAI extractor with model: {self.model}, batch: {self.use_batch}")
    
    def extract_memories(self, text_chunk: str, conversation_title: str = "") -> List[ExtractedMemory]:
    ...
    def _add_to_batch(self, text_chunk: str, conversation_title: str) -> List[ExtractedMemory]:
        ...
        batch_request = {
            ...
            "body": {
                "model": self.model,
                "messages": [
                    {"role": "system", "content": "You are an expert at extracting personal memories and preferences from conversations. Always return valid JSON."},
                    {"role": "user", "content": self._create_extraction_prompt(text_chunk, conversation_title)}
                ],
                "temperature": 0.3,
                "max_tokens": 2000,
                "top_p": 0.9
    ...
    def process_batch(self) -> List[ExtractedMemory]:
        ...
        try:
            ...
            # Upload batch file
            self.logger.info(f"Uploading batch file with {len(self.batch_requests)} requests...")
        ...
        for line in results.strip().split('\n'):
            ...
            try:
                ...
                if custom_id in request_map:
                    ...
                    memories = self._parse_openai_response(response_content, context_info["context"])
    ...
    def _create_extraction_prompt(self, text_chunk: str, conversation_title: str) -> str:
...
Extract memories:"""
    ...
    def _extract_with_openai(self, text_chunk: str, conversation_title: str) -> List[ExtractedMemory]:
    ...
    def _parse_openai_response(self, response: str, context: str) -> List[ExtractedMemory]:
